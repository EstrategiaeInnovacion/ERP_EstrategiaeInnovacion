...
@section('title', 'Gestionar Dependencias - Legal')
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <div class="flex items-center gap-2 text-sm text-slate-500 mb-1">
                ...
                <span>/</span>
                <a href="{{ route('legal.matriz.index') }}" class="hover:text-amber-600 transition-colors">Matriz de Consultas y Escritos</a>
                <span>/</span>
                <span class="text-slate-700 font-medium">Dependencias</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900">Gestionar Dependencias</h1>
            <p class="text-slate-500 mt-1 text-sm">Administra las dependencias de la Matriz de Consultas y Escritos.</p>
        </div>
        ...
        <div id="panelcat-consulta" class="bg-white rounded-b-2xl rounded-tr-2xl border border-slate-200 shadow-sm overflow-hidden">
            @if($catsConsulta->isEmpty())
                <div class="text-center py-16 text-slate-400">
                    ...
                    <p class="text-sm">No hay dependencias de consulta creadas aún.</p>
                </div>
            @else
                    ...
                    <li class="px-6 py-3">
                        <div class="flex items-center justify-between">
                            ...
                            <form action="{{ route('legal.categorias.destroy', $cat->id) }}" method="POST"
                                  onsubmit="return confirm('¿Eliminar la dependencia \'{{ addslashes($cat->nombre) }}\'?')">
                                @csrf @method('DELETE')
        ...
        <div id="panelcat-escritos" class="hidden bg-white rounded-b-2xl rounded-tr-2xl border border-slate-200 shadow-sm overflow-hidden">
            @if($catsEscritos->isEmpty())
                <div class="text-center py-16 text-slate-400">
                    ...
                    <p class="text-sm">No hay dependencias de escritos creadas aún.</p>
                </div>
            @else
                <ul class="divide-y divide-slate-100">
