                    ...
                    <div class="p-4 hover:bg-slate-50 transition-colors {{ !$recordatorio->leido ? 'bg-amber-50/30' : '' }}">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-shrink-0 flex items-center gap-2">
                                ...
                                @endif
                                <form action="{{ route('rh.recordatorios.destruir', $recordatorio->id) }}" method="POST" onsubmit="return confirm('¿Descartar este recordatorio?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Descartar">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
