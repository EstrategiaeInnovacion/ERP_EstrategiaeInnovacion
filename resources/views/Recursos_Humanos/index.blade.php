    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            @php
                ...
                $posNorm = $emp ? mb_strtolower(preg_replace('/\s+/u', ' ', trim($emp->posicion ?? '')), 'UTF-8') : null;
                $esRhAdmin = $posNorm && in_array($posNorm, ['direccion', 'administracion rh', 'ti']);
                $tieneSubordinados = $emp && $emp->subordinados()->where('es_activo', true)->exists();
                $rutaReloj = ($tieneSubordinados && !$esRhAdmin) ? route('rh.reloj.equipo') : route('rh.reloj.index');
            @endphp
            <a href="{{ $rutaReloj }}" class="group relative bg-white rounded-3xl p-8 shadow-sm border border-slate-200 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col">
