...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-slate-900">Editar Día Festivo</h1>
        <p class="text-slate-500 mt-1">Modifica los datos del día festivo o inhábil</p>
    </div>

    {{-- Formulario --}}
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <form method="POST" action="{{ route('rh.dias-festivos.update', $diaFestivo->id) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="nombre" id="nombre" 
                       value="{{ old('nombre', $diaFestivo->nombre) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="fecha" id="fecha" 
                           value="{{ old('fecha', $diaFestivo->fecha->format('Y-m-d')) }}"
                           class="w-full rounded-xl border-slate-300 text-sm focus:ring-indigo-500 focus:border-indigo-500"
                           required>
                    @error('fecha')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="tipo" id="tipo" 
                        ...
                        <option value="festivo" {{ old('tipo', $diaFestivo->tipo) === 'festivo' ? 'selected' : '' }}>Día Festivo</option>
                        <option value="inhabil" {{ old('tipo', $diaFestivo->tipo) === 'inhabil' ? 'selected' : '' }}>Día Inhábil</option>
                    </select>
                    @error('tipo')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox" name="es_anual" id="es_anual" 
                       value="1" {{ old('es_anual', $diaFestivo->es_anual) ? 'checked' : '' }}
            ...
            <div>
                ...
                <textarea name="descripcion" id="descripcion" rows="4"
                          class="w-full rounded-xl border-slate-300 text-sm focus:ring-indigo-500 focus:border-indigo-500"
                          placeholder="Mensaje que recibirán los empleados en la notificación...">{{ old('descripcion', $diaFestivo->descripcion) }}</textarea>
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox" name="activo" id="activo" 
                       value="1" {{ old('activo', $diaFestivo->activo) ? 'checked' : '' }}
                       class="w-5 h-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
