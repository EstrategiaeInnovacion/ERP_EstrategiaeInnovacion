...
@section('title', 'Editar Registro - Contraseñas y Equipos')
    ...
    <div class="bg-white border-b border-slate-200 mb-8">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex items-center gap-3 mb-1">
                ...
                <span class="text-slate-600 text-sm font-medium">Editar Registro #{{ $credencial->id }}</span>
            </div>
            <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Editar Registro</h1>
            <p class="text-slate-500 mt-1">{{ $credencial->user->name ?? 'Usuario' }} — {{ $credencial->equipo_asignado }}</p>
        </div>
    </div>

    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
            <form method="POST" action="{{ route('admin.credenciales.update', $credencial) }}" class="divide-y divide-slate-100">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <select id="user_id" name="user_id"
                                ...
                                @foreach($usuarios as $usuario)
                                    <option value="{{ $usuario->id }}"
                                        {{ old('user_id', $credencial->user_id) == $usuario->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="text" id="nombre_usuario_sistema" name="nombre_usuario_sistema"
                                   value="{{ old('nombre_usuario_sistema', $credencial->nombre_usuario_sistema) }}"
                        ...
                        <div>
                            <label for="contrasena" class="block text-sm font-semibold text-slate-700 mb-1.5">
                                Nueva contraseña
                                <span class="text-xs font-normal text-slate-400">(dejar vacío para no cambiar)</span>
                            </label>
                            <div class="relative">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" id="equipo_asignado" name="equipo_asignado"
                                   value="{{ old('equipo_asignado', $credencial->equipo_asignado) }}"
                        ...
                        <div>
                            ...
                            <select id="tipo_equipo" name="tipo_equipo"
                                ...
                                @foreach($tipos as $tipo)
                                    <option value="{{ $tipo }}"
                                        {{ old('tipo_equipo', $credencial->tipo_equipo) === $tipo ? 'selected' : '' }}>
                                        {{ $tipo }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" id="numero_serie" name="numero_serie"
                                   value="{{ old('numero_serie', $credencial->numero_serie) }}"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" id="sistema_operativo" name="sistema_operativo"
                                   value="{{ old('sistema_operativo', $credencial->sistema_operativo) }}"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea id="observaciones" name="observaciones" rows="3"
                                      class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition resize-none">{{ old('observaciones', $credencial->observaciones) }}</textarea>
                        </div>
                    </div>
                </div>

                {{-- Botones --}}
                <div class="px-8 py-5 bg-slate-50/50 flex justify-between items-center">
