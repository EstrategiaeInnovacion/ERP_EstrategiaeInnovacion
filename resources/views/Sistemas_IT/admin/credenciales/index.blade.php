...
@endpush
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {{-- Buscador --}}
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 mb-6 p-4">
            <form method="GET" action="{{ route('admin.credenciales.index') }}" class="flex gap-3">
                <div class="flex-1 relative">
                    <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0"/>
                    </svg>
                    <input type="text" name="search" value="{{ request('search') }}"
                           placeholder="Buscar por usuario, equipo o número de serie..."
                           class="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-700 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition">
                </div>
                <button type="submit"
                        class="px-5 py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 transition">
                    Buscar
                </button>
                @if(request('search'))
                    <a href="{{ route('admin.credenciales.index') }}"
                       class="px-4 py-2.5 bg-slate-100 text-slate-600 text-sm font-medium rounded-xl hover:bg-slate-200 transition">
                        Limpiar
                    </a>
                @endif
            </form>
        ...
        <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                <div>
                    <h3 class="text-lg font-bold text-slate-900">Registros</h3>
                    <p class="text-sm text-slate-500 mt-0.5">
                        {{ $credenciales->total() }} {{ Str::plural('registro', $credenciales->total()) }} encontrado{{ $credenciales->total() !== 1 ? 's' : '' }}
                    </p>
                </div>
                <div class="flex items-center gap-2">
                    <div class="w-2 h-2 rounded-full bg-indigo-500"></div>
                    <span class="text-xs text-slate-400 font-medium uppercase tracking-wider">Solo visible para Administradores</span>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                @if($credenciales->isEmpty())
                    <div class="py-20 text-center">
                        ...
                        <p class="text-slate-400 font-medium">No hay registros aún.</p>
                        <a href="{{ route('admin.credenciales.create') }}"
                           class="mt-4 inline-flex items-center text-sm text-indigo-600 hover:text-indigo-700 font-semibold">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-100">
                                ...
                                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Login de Sistema</th>
                                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Equipo Asignado</th>
                                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Tipo</th>
                                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">No. Serie</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @foreach($credenciales as $credencial)
                                <tr class="hover:bg-slate-50/60 transition-colors group">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p class="text-sm font-bold text-slate-900 leading-tight">
                                                    {{ $credencial->user->name ?? 'Usuario eliminado' }}
                                                </p>
                                                <p class="text-xs text-slate-400">{{ $credencial->user->email ?? '' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <p class="text-sm font-semibold text-slate-800">{{ $credencial->equipo_asignado }}</p>
                                        @if($credencial->sistema_operativo)
                                            <p class="text-xs text-slate-400 mt-0.5">{{ $credencial->sistema_operativo }}</p>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4">
                                        @php
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold {{ $tipoBadge }}">
                                            {{ $credencial->tipo_equipo }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-slate-500 font-mono">
                                        {{ $credencial->numero_serie ?? '—' }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            {{-- Visualizar --}}
                                            <a href="{{ route('admin.credenciales.show', $credencial) }}"
                                               title="Visualizar"
                                            ...
                                            {{-- Eliminar --}}
                                            <form method="POST"
                                                  action="{{ route('admin.credenciales.destroy', $credencial) }}"
                                                  onsubmit="return confirm('¿Confirmas eliminar este registro? Esta acción no se puede deshacer.')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                        title="Eliminar"
