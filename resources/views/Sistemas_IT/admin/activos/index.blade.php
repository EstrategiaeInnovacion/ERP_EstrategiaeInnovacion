                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @if($d->status === 'broken')
                                    <form method="POST" action="{{ route('admin.activos.destroy', $d->uuid) }}"
                                          onsubmit="return confirm('¿Eliminar permanentemente el dispositivo «{{ addslashes($d->name) }}»? Esta acción no se puede deshacer.')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                                class="inline-flex items-center px-2.5 py-1.5 bg-red-50 text-red-700 text-xs font-semibold rounded-lg hover:bg-red-100 transition"
                                                title="Eliminar dispositivo dañado">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
