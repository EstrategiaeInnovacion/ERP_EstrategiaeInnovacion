...
echo "╔════════════════════════════════════════════════════════════════════════╗\n";
echo "║  RESTAURAR BD OLD desde BD LOCAL (estrategias_innovacion)              ║\n";
echo "║  Campos: id_empleado, posicion, area                                   ║\n";
echo "╚════════════════════════════════════════════════════════════════════════╝\n\n";

// Obtener empleados de la BD LOCAL que tienen datos (no NULL)
...
echo "Empleados en BD LOCAL con datos: " . $empleadosLocal->count() . "\n";
...
foreach ($empleadosLocal as $empLocal) {
    ...
    if ($empOld) {
        ...
        // Solo copiar si el valor local NO es NULL
        if (!is_null($empLocal->id_empleado)) {
            $cambios['id_empleado'] = $empLocal->id_empleado;
        }
        if (!is_null($empLocal->posicion)) {
            $cambios['posicion'] = $empLocal->posicion;
        }
        if (!is_null($empLocal->area)) {
            $cambios['area'] = $empLocal->area;
        }
        ...
        if (!empty($cambios)) {
            ...
            echo "  → Encontrado como: {$empOld->nombre} (ID: {$empOld->id})\n";
...
echo "║  RESUMEN                                                               ║\n";
echo "╚════════════════════════════════════════════════════════════════════════╝\n";
echo "✓ Actualizados en BD OLD: {$actualizados}\n";
